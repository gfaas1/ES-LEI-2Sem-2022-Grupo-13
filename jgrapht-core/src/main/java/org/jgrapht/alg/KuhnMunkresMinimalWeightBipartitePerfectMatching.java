...
/**
 * Kuhn-Munkres algorithm (named in honor of Harold Kuhn and James Munkres) solving <i>assignment problem</i> also known
 * as <a href=http://en.wikipedia.org/wiki/Hungarian_algorithm>hungarian algorithm</a> (in the honor of hungarian
 * mathematicians Dénes K?nig and Jen? Egerváry). It's running time O(V^3).
 *
 * <p>
 * <i>Assignment problem</i> could be set as follows:
 * <p>
 * Given <a href=http://en.wikipedia.org/wiki/Complete_bipartite_graph>complete bipartite graph</a> G = (S, T; E),
 * such that |S| = |T|, and each edge has <i>non-negative</i> cost <i>c(i, j)</i>, find <i>perfect</i> matching of
 * <i>minimal cost<i/>.
 * </p>
 * </p>
 *
...
public class KuhnMunkresMinimalWeightBipartitePerfectMatching<V, E> implements WeightedMatchingAlgorithm<V, E> {
    ...
    public Set<E> getMatching() {
        ...
        for (int i=0; i < matching.length; ++i) {
          edges.add(
            graph.getEdge(
              firstPartition.get(i), secondPartition.get(matching[i])
            )
          );
        }
    ...
    protected static class KuhnMunkresMatrixImplementation<V, E> {
        ...
        public KuhnMunkresMatrixImplementation(final WeightedGraph<V, E> G, final List<? extends V> S, final List<? extends V> T) {

            int partition = S.size();

            // Build an excess-matrix corresponding to the supplied weighted complete bipartite graph
        ...
        /**
         * Gets costs-matrix as input and returns assignment of tasks (designated by the columns of cost-matrix)
         * to the workers (designated by the rows of the cost-matrix) so that to MINIMIZE total tasks-tackling costs
         */
        protected int[] buildMatching() {

            int height  = costMatrix.length,
                width   = costMatrix[0].length;
        ...
        double[][] makeExcessMatrix() {
            ...
            //
            // NOTE:  Makes nothing if there is any worker that can more (cost-)effectively
            //        tackle this task than any other, i.e. there is any row having zero in this
            //        column. However, if there is no one, reduce the cost-demands of each worker to
            //        the size of the min-cost (we can easily do this, since we have particular interest
            //        of the relative values only), i.e. subtract the value of the minimum cost-demands
            //        to highlight (with zero) the worker that can tackle this task demanding lowest reward.
        ...
        int buildMaximalMatching() {
            ...
            //
            // As to E. Burge:
            //    Matching is maximal iff bipartite graph doesn't contain any matching-augmenting paths.
            //
            ...
            while(extending && matchingSize < excessMatrix.length) {
              ...
              for(int j=0; j < excessMatrix.length; ++j) {
                if (rowMatched[j] == -1 && !colsVisited[j]) {
                  extending |= new MatchExtender(rowsVisited, colsVisited).extend(j);  /* Try to extend matching */
                }
              }
        ...
        void buildVertexCoverage() {
            ...
            while (cont) {
                ...
                for (int i = 0; i < excessMatrix.length; ++i) {
                    if (rowsCovered[i])
                        for (int j = 0; j < excessMatrix[i].length; ++j) {
                            if (Double.valueOf(excessMatrix[i][j]).compareTo(0.) == 0 && !columnsCovered[j]) {
         ...
         */
        private static boolean minimal(final int[] match, final boolean[] rowsCovered, final boolean[] colsCovered) {
         ...
         * @param colsCovered     columns coverage to check
         */
        private static int uncovered(final double[][] excessMatrix, final boolean[] rowsCovered, final boolean[] colsCovered) {
            ...
            for (int i = 0; i < excessMatrix.length; ++i) {
                if (rowsCovered[i]) continue;
                for (int j = 0; j < excessMatrix[i].length; ++j) {
                    if (colsCovered[j]) continue;
                    if (Double.valueOf(excessMatrix[i][j]).compareTo(0.) == 0) ++uncoveredZero;
                }
            }
        ...
        protected class MatchExtender {
            ...
            private boolean extendMatchingOL(int pathTailRow, int pathTailCol) {
                ...
                if (columnMatched[pathTailRow] == -1) {
                    ...
                    // There is no way to continue: mark zero as matched, trigger along-side flipping
                    ...
                    if (colsVisited[columnMatched[pathTailRow]]) return false;

                    boolean extending = extendMatchingEL(
