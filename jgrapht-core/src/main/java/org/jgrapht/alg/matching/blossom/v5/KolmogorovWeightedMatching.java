...
/**
 * This class computes weighted matchings in general graphs. Depending on the constructor parameter the
 * weight of the resulting matching is maximized or minimized. If maximum of minimum weight <i>perfect</i>
 * algorithm is needed, see {@link KolmogorovWeightedPerfectMatching}.
 * <p>
 * This class reduces both maximum and minimum weight matching problems to the maximum and minimum weight
 * perfect matching problems correspondingly. See {@link KolmogorovWeightedPerfectMatching} for relative
...
public class KolmogorovWeightedMatching<V, E> implements MatchingAlgorithm<V, E> {
     ...
     * algorithm is to minimize the weight of the resulting matching.
     *
    ...
    /**
     * Constructs a new instance of the algorithm using the default options. The goal of the constructed
     * algorithm is to maximize or minimize the weight of the resulting matching depending on
    ...
    /**
     * Constructs a new instance of the algorithm with the specified {@code options}. The goal of the constructed
     * algorithm is to minimize the weight of the resulting matching.
     *
     * @param initialGraph the graph for which to find a weighted matching
    ...
    /**
     * Computes and returns a matching of maximum or minimum weight in the {@code initialGraph} depending on the
    ...
    private void lazyComputeMaximumWeightMatching() {
        ...
        Graph<V, E> graphCopy = GraphTypeBuilder.undirected().
                ...
                allowingSelfLoops(type.isAllowingSelfLoops()).vertexSupplier(initialGraph.getVertexSupplier()).
        ...
        for (E edge : initialGraph.edgeSet()) {
            Graphs.addEdgeWithVertices(graphCopy, duplicatedVertices.get(initialGraph.getEdgeSource(edge)), duplicatedVertices.get(initialGraph.getEdgeTarget(edge)), initialGraph.getEdgeWeight(edge));
        }
        ...
        this.graph = new AsGraphUnion<>(new AsWeightedGraph<>(graphCopy, zeroWeightFunction), initialGraph);
        this.perfectMatching = new KolmogorovWeightedPerfectMatching<>(graph, options, objectiveSense);
     ...
     * {@link KolmogorovWeightedPerfectMatching#getError()}. More precisely, the total error
     * equals the sum of:
     * <ul>
