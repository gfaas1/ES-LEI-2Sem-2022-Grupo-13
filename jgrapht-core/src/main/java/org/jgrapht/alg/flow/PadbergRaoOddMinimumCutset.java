...
/**
 * Implementation of the algorithm by Padberg and Rao to compute Odd Minimum Cut-Sets. Let G=(V,E) be an undirected, simple weighted graph, where all edge weights are positive. Let T &sub; V
 * with |T| even, be a set of vertices that are labelled <i>odd</i>. A cut-set (U:V-U) is called odd if |T &cap; U| is an odd number. Let c(U:V-U) be the weight of the cut, that is, the sum
 * of weights of the edges which have exactly one endpoint in U and one endpoint in V-U. The problem of finding an odd minimum cut-set in G is stated as follows:
 * Find W &sube; V such that c(W:V-W)=min{c(U:V-U)|U&sube; V, |T &cap; U| is odd}.
 *
 * <p>The algorithm has been published in: Padberg, M. Rao, M. Odd Minimum Cut-Sets and b-Matchings. Mathematics of Operations Research, 7(1), p67-80, 1982. A more concise description is
 * published in: Letchford, A. Reinelt, G. Theis, D. Odd minimum cut-sets and b-matchings revisited. SIAM Journal of Discrete Mathematics, 22(4), p1480-1487, 2008.
 *
 * <p>The runtime complexity of the algorithm, although not formally stated, is roughly O(|V|+Q), where Q is the runtime complexity of the algorithm used to compute a Gomory-Hu tree on graph G.
 *
...
public class PadbergRaoOddMinimumCutset<V,E>{
     ...
     */
    public PadbergRaoOddMinimumCutset(SimpleWeightedGraph<V, E> network, MinimumSTCutAlgorithm<V,E> minimumSTCutAlgorithm) {
        this.network = network;
        gusfieldGomoryHuCutTreeAlgorithm=new GusfieldGomoryHuCutTree<>(network, minimumSTCutAlgorithm);
    }

    /**
     * Calculates the minimum odd cut. The implementation follows Algorithm 1 in the paper Odd minimum cut sets
     * and b-matchings revisited by Adam Letchford, Gerhard Reinelt and Dirk Theis. The original algorithm runs on a compressed
     * Gomory-Hu tree: a cut-tree with the odd vertices as terminal vertices. This tree has |T|-1 edges as opposed to |V|-1
     * for a Gomory-Hu tree defined on the input graph G.
     * This compression step can however be skipped. If you want to run the original algorithm in the paper (default), set the parameter
     * <code>useTreeCompression</code> to true. Alternatively, experiment which setting of this parameter produces the fastest
     * results. Both settings are guaranteed to find the optimal cut. Experiments on random graphs showed that setting <code>useTreeCompression</code> to true was on average a bit faster,
     * but there were some cases where setting <code>useTreeCompression</code> to false yielded significantly faster results.
     *
     * @param oddVertices Set of vertices which are labeled 'odd'. Note that the number of vertices in this set must be even!
     * @param useTreeCompression parameter indicating whether tree compression should be used (default: true).
    ...
    /**
     * Modified implementation of the algorithm proposed in Odd Minimum Cut-sets and b-matchings by Padberg and Rao. The optimal cut is directly computed on the Gomory-Hu tree computed
     * for graph G. This approach iterates efficiently over all possible cuts of the graph (there are |V| such cuts).
     * @param oddVertices Set of vertices which are labeled 'odd'. Note that the number of vertices in this set must be even!
     * @return weight of the minimum odd cut.
     */
    private double calculateMinCutWithoutTreeCompression(Set<V> oddVertices){
        ...
        for(DefaultWeightedEdge edge : edges){
            ...
            if(edgeWeight < minimumCutWeight && PadbergRaoOddMinimumCutset.isOddSet(sourcePartition, oddVertices)) { //If the source partition forms an odd cutset, check whether the cut isn't better than the one we already found.
    ...
    /**
     * Implementation of the algorithm proposed in Odd Minimum Cut-sets and b-matchings by Padberg and Rao. The algorithm first compresses the Gomory-Hu tree, thereby creating a cut-tree
     * using the odd vertices as terminal vertices.
     * @param oddVertices Set of vertices which are labeled 'odd'. Note that the number of vertices in this set must be even!
     * @return weight of the minimum odd cut.
     */
    private double calculateMinCutWithTreeCompression(Set<V> oddVertices){
        ...
        assert network.vertexSet().containsAll(oddVertices); //All odd vertices must be contained in the graph
        assert oddVertices.size()%2==0; //Number of odd vertices needs to be even
        assert network.edgeSet().stream().filter(e -> network.getEdgeWeight(e) < 0).count()==0; //All edge weights need to be positive

        //Build a Gomory-Hu tree on the entire graph
        gomoryHuTree=gusfieldGomoryHuCutTreeAlgorithm.getGomoryHuTree();

        //Optional: Compress the graph (computations could be performed on the original Gomory-Hu tree)
        SimpleWeightedGraph<Cluster, DefaultWeightedEdge> compositeTree=this.createCompositeTree();
    ...
    /**
     * Creates a compressed Gomory-Hu tree consisting of 'cluster' nodes. Each cluster node contains exactly one odd node, and zero or more even nodes.
     * In the paper, this graph is defined as a cut-tree on the original graph G with terminal vertex set T.
    ...
    private SimpleWeightedGraph<Cluster, DefaultWeightedEdge> createCompositeTree(){
        SimpleWeightedGraph<Cluster, DefaultWeightedEdge> compressedTree=new SimpleWeightedGraph<>(DefaultWeightedEdge.class);

        //Initialize
        //Choose 2 random odd nodes (there are certainly 2 odd nodes since the number of odd nodes in the graph must be even).
        ...
        List<DefaultWeightedEdge> pathEdges= DijkstraShortestPath.findPathBetween(gomoryHuTree, oddNode1, oddNode2);
        DefaultWeightedEdge cheapestEdge=pathEdges.stream().min(Comparator.comparing(gomoryHuTree::getEdgeWeight)).orElseThrow(()->new RuntimeException("path should not be empty?!"));
        ...
        ConnectivityInspector<V, DefaultWeightedEdge> connectivityInspector=new ConnectivityInspector<>(gomoryHuTree);
        ...
        if(cluster1.oddNodeSubset.size() > 1) //Place the node back onto the queue if it contains multiple odd nodes
            openNodes.add(cluster1);
        if(cluster2.oddNodeSubset.size() > 1) //Place the node back onto the queue if it contains multiple odd nodes
        ...
        while(!openNodes.isEmpty()){
            ...
            if(parent.oddNodeSubset.size() > 1) //Place the node back onto the queue if it contains multiple odd nodes
    ...
    /**
     * Procedure which takes a cluster (parent) with 2 or more odd nodes and splits off a new cluster. The new cluster contains at least one odd node.
     * As a result, the parent cluster has at least one less
     ...
     */
    private Cluster splitNode(SimpleWeightedGraph<Cluster, DefaultWeightedEdge> compressedTree, Cluster parent){
        assert parent.oddNodeSubset.size() >= 2;

        //Choose 2 random odd nodes (there are certainly 2 odd nodes as this is guaranteed by the invoking function).
        ...
        //Remove the selected edge from the gomoryHuTree graph. This splits the component the edge was part of into two separate components.
        gomoryHuTree.removeEdge(cheapestEdge);
        ...
        //One of the components contains the cluster head of the parent. This component is used to update the parent, the other component becomes a new cluster.
        Cluster child;
    ...
    /**
     * Function which evaluates the value (weight) of a cut. If the cut is better than the best cut observed thus far,
     ...
     */
    private void evaluateCut(SimpleWeightedGraph<Cluster, DefaultWeightedEdge> forestGraph, double cutWeight){
        //Check whether the proposed cut can improve uppon the best cut found thus far. If not, return.
        if(minimumCutWeight < cutWeight)
            return;

        ConnectivityInspector<Cluster, DefaultWeightedEdge> connectivityInspector=new ConnectivityInspector<>(forestGraph);
        ...
        //Test whether we have found a parition with an odd number of odd nodes (each composite node contains exactly one odd node)
     ...
     * @return partition W
     */
    public Set<V> getSourcePartition(){ return sourcePartitionMinimumCut;}
    ...
    /**
     * Returns the set of edges which run from the source partition to the sink partition, in the s-t cut obtained after the last
     * invocation of {@link #calculateMinCut(Set, boolean)}
     *
     * @return set of edges which have one endpoint in the source partition and one endpoint in the sink partition.
     */
    ...
    private class Cluster {
        ...
        private Cluster(V clusterHead, Set<V> nodes) {
            ...
            this.nodes=nodes;
            oddNodeSubset=nodes.stream().filter(oddVertices::contains).collect(Collectors.toSet());
        }
