 ...
 * undirected graph.
 *
 * The algorithm was developed by Czech mathematician V. Jarník and later independently by computer scientist Robert C. Prim
 * and rediscovered by E. Dijkstra.
 *
...
public class PrimMinimumSpanningTree<V, E> implements MinimumSpanningTree<V, E> {
    ...
    public PrimMinimumSpanningTree(final Graph<V, E> g) {
        ...
        while (!unspanned.isEmpty()) {
            ...
            // Edges crossing the cut C = (S, V \ S), where S is set of already spanned vertices
            ...
            for (E next; (next = dangling.poll()) != null;) {
                ...
                // Decayed edges aren't removed from priority-queue so that having them just ignored
                // being encountered through min-max traversal
