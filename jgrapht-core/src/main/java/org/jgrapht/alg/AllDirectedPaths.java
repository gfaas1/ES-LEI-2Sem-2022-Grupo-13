...
/**
 * A Dijkstra-like algorithm to find all paths between two sets of
 * nodes in a directed graph, with options to search only simple paths
 * and to limit the path length.
 *
...
public class AllDirectedPaths<V, E> {
    ...
    /**
     * Calculate (and return) all paths from the source vertex to the
     * target vertex.
     *
     ...
     * (if null, all paths are considered, which may be very slow due
     * to potentially huge output)
     **/
    ...
    /**
     * Calculate (and return) all paths from the source vertices to
     * the target vertices.
     *
     ...
     * to potentially huge output)
     *
     * @return list of all paths from the sources to the targets
     * containing no more than maxPathLength edges
     **/
    ...
    /**
     * Compute the minimum number of edges in a path to the targets
     * through each edge, so long as it is not greater than a bound.
     *
     * @param targetVertices the target vertices
     * @param maxPathLength maximum number of edges to allow in a path
     * (if null, all edges will be considered, which may be expensive)
     *
     * @return the minimum number of edges in a path from each edge to
     * the targets, encoded in a Map
     **/
    private Map<E, Integer> edgeMinDistancesBackwards(Set<V> targetVertices,
        ...
        for (V vertex; (vertex = verticesToProcess.poll()) != null; ) {
            ...
            // Check whether the incoming edges of this node are correctly decorated
    ...
    /**
     * Generate all paths from the sources to the targets, using
     * pre-computed minimum distances.
     *
     * @param sourceVertices the source vertices
     * @param targetVertices the target vertices
     * @param maxPathLength maximum number of edges to allow in a path
     * @param simplePathsOnly if true, only search simple
     * (non-self-intersecting) paths (if null, all edges will be
     * considered, which may be expensive)
     * @param edgeMinDistancesFromTargets the minimum number of edges
     * in a path to a target through each edge, as computed by {@code
     * edgeMinDistancesBackwards}.
     *
     * @return a List of all GraphPaths from the sources to the
     * targets satisfying the given constraints
     **/
            ...
            for (E outEdge: graph.outgoingEdgesOf(leafNode)) {
                ...
                if (edgeMinDistancesFromTargets.containsKey(outEdge) &&
                    ...
                    if ((maxPathLength == null) || (newPath.size() < maxPathLength)) {
                        incompletePaths.addFirst(newPath); // We use incompletePaths in FIFO mode to avoid memory blowup
                    }
                }
            }
        }
