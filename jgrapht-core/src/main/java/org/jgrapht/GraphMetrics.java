...
public abstract class GraphMetrics {
     ...
     * the length (number of edges) of the smallest cycle in the graph. Acyclic graphs are considered to have infinite girth. For directed
     * graphs, the length of the shortest directed cycle is returned (see Bang-Jensen, J., Gutin, G., Digraphs: Theory, Algorithms and Applications, Springer Monographs in Mathematics, ch 1, ch 8.4.).
     * Simple undirected graphs have a girth of at least 3 (triangle cycle). Directed graphs and Multigraphs have a girth of
     * at least 2 (parallel edges/arcs), and in Pseudo graphs have a girth of at least 1 (self-loop).
     * <p>
     * This implementation is loosely based on these <a href="http://webcourse.cs.technion.ac.il/234247/Winter2003-2004/ho/WCFiles/Girth.pdf">notes</a>.
     * In essence, this method invokes a Breadth-First search from every vertex in the graph. A single Breadth-First search takes $O(n+m)$ time,
     * where $n$ is the number of vertices in the graph, and $m$ the number of edges.
     * Consequently, the runtime complexity of this method is $O(n(n+m))=O(mn)$.
     * <p>
     * An algorithm with the same worst case runtime complexity, but a potentially better average runtime complexity of $O(n^2)$ is described in:
     * Itai, A. Rodeh, M. Finding a minimum circuit in a graph. SIAM J. Comput. Vol 7, No 4, 1987.
     * @param graph input graph
     * @param <V> graph vertex type
     * @param <E> graph edge type
    ...
    public static <V,E> int getGirth(Graph<V,E> graph){
        ...
        if(graph.getType().isUndirected()) {
            ...
            //Start a BFS search tree from each vertex. The search stops when a triangle (smallest possible cycle) is found.
            ...
            for (int i = 0; i < vertices.size() - 2 && girth > 3; i++) {
                ...
                do {
                    ...
                    for (V v : neighborIndex.neighborsOf(u)) {
                        ...
                        if (parent[indexU] == indexV) { //Skip the parent of vertex u, unless there are multiple edges between u and v
        ...
        assert graph.getType().isUndirected() && graph.getType().isSimple() && girth >= 3 ||
                graph.getType().isAllowingSelfLoops() && girth >= 1 ||
                girth >= 2 && (graph.getType().isDirected() || graph.getType().isAllowingMultipleEdges());
        return girth;
    }
}
