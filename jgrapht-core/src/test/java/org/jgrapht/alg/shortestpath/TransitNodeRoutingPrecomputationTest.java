...
public class TransitNodeRoutingPrecomputationTest {
     ...
     * the set of sources and targets of many-to-many shortest paths are equal to the
     * set of transit vertices. Fourthly, checks that cell ids in the Voronoi diagram
     * are in range $[-1, |V| - 1]$. Finally, checks that paths for access vertices
     * are computed correctly.
     *
     * @param graph   graph
     * @param routing transit node routing
     */
    private void assertCorrectTNR(Graph<Integer, DefaultWeightedEdge> graph,
        ...
        for (ContractionVertex<Integer> vertex : routing.getTransitVertices()) {
            assertTrue(vertex.contractionLevel >= numberOfVertices - routing.getTransitVertices().size());
        }

        // many-to-many shortest paths
        Set<Integer> transitVerticesSet = routing.getTransitVertices().stream()
        ...
        for (ContractionVertex<Integer> vertex : routing.getContractionHierarchy().getContractionGraph().vertexSet()) {
        ...
        AccessVertices<Integer, DefaultWeightedEdge> accessVertices = routing.getAccessVertices();
        ShortestPathAlgorithm<Integer, DefaultWeightedEdge> sp = new BidirectionalDijkstraShortestPath<>(graph);
        for (ContractionVertex<Integer> vertex : routing.getContractionHierarchy().getContractionGraph().vertexSet()) {
            List<AccessVertex<Integer, DefaultWeightedEdge>> av = accessVertices.getForwardAccessVertices(vertex);
            for (AccessVertex<Integer, DefaultWeightedEdge> accessVertex : av) {
                assertEquals(sp.getPath(vertex.vertex, accessVertex.getVertex()), accessVertex.getPath());
            }
        }
        for (ContractionVertex<Integer> vertex : routing.getContractionHierarchy().getContractionGraph().vertexSet()) {
            List<AccessVertex<Integer, DefaultWeightedEdge>> av = accessVertices.getBackwardAccessVertices(vertex);
            for (AccessVertex<Integer, DefaultWeightedEdge> accessVertex : av) {
                assertEquals(sp.getPath(accessVertex.getVertex(), vertex.vertex), accessVertex.getPath());
            }
        }
    }
