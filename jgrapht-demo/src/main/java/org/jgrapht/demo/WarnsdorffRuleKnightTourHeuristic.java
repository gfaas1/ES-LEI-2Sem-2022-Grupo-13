 ...
 *
 * Description of the Warnsdorff's rule: set a start cell.
 * Always proceed to the cell that have the fewest onward moves.
 * In case of a tie(i.e. there exist more than one possible choice for the next cell)
 * go to the cell with largest Euclidean distance from the center of the board.
 *
 ...
 *
 * 1). $(1, 0) \to (0, 2)$ - denoted as $1$ on the picture below.
 * 2). $(2, 0) \to (0, 1)$ - denoted as $2$ on the picture below.
 * 3). $(n - 3, 0) \to (n - 1, 1)$ - denoted as $3$ on the picture below.
 * 4). $(n - 2, 0) \to (n - 1, 2)$ - denoted as $4$ on the picture below.
 * 5). $(0, m - 3) \to (1, m - 1)$ - denoted as $5$ on the picture below.
 * 6). $(0, m - 2) \to (2, m - 1)$ - denoted as $6$ on the picture below.
 * 7). $(n - 3, m - 1) \to (n - 1, m - 2)$ - denoted as $7$ on the picture below.
 * 8). $(n - 2, m - 1) \to (n - 1, m - 3)$ - denoted as $8$ on the picture below.
 *
 * #########################################
 * #*12*********************************34*#
 * #2*************************************3#
...
public class WarnsdorffRuleKnightTourHeuristic {
    ...
    private boolean checkExistence(TourType type) {
        ...
        /*
         ...
         Theorem: An n x m chessboard with n <= m has a closed knight's tour
        ...
        /*
         Regarding open knight's tour existence, refer to http://gaebler.us/share/Knight_tour.html.
     ...
     *
     * Warnsdorff's rule heuristic is an example of a greedy method,
     * which we use to select the next cell to move, and thus may fail to find a tour.
     * However, another greedy heuristic is used to prevent failing:
     * in case of a tie we will select a cell with the largest euclidean distance from the center of the board.
     * Such combination of greedy methods significantly increases our chances to find a tour.
     *
     * @param type of the tour.
    ...
    public KnightTour getTour(TourType type, boolean structured, int shiftX, int shiftY) {
        ...
        while (!found) {
            ...
            if (visited == n * m && checkType(startX, startY, endCell.getFirst(), endCell.getSecond(), type)) {
                HashSet<Pair<Pair<Integer, Integer>, Pair<Integer, Integer>>> moves = getMoves(tour.getList());
